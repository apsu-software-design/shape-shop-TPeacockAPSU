...
function letUserSelectItem() {
  ...
  4. Go back. Don't buy anything.`);
    ...
    switch (response) { //handle each response
        case '1': shopModel.addProduct("Triangle", 3.5, "It's got three sides!"); break;
        case '2': shopModel.addProduct("Square", 4.5, "It's got four sides!"); break;
        case '3': shopModel.addProduct("Pentagon", 5.5, "It's got five sides!"); break;
        default: console.log('Invalid option!');
    }
    console.log(''); //extra empty line for revisiting
}
...
function letUserSelectQuantity() {
  ...
  `);
    ...
    let response = readlineSync.question('> ')
    shopModel.addQuantity(parseInt(response));
    console.log(''); //extra empty line for revisiting
}

function removeItemFromCart() {
    let shopModel = new ShopModel;
    let products = new ProductView(shopModel);

    console.log(`Select an item to be removed from the cart.`);

    console.log(products.getNameView());

    let response = readlineSync.question('> ')
    let removeMe = parseInt(response);

    shopModel.removeProduct(removeMe);
...
function viewCartTotal() {
    ...
    console.log(products.getPriceView);
}
